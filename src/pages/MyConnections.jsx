  ...
  return (
    <div className="px-2 md:px-3">
      <div className="mx-auto max-w-[1440px]">
        <h1 className="py-4 text-lg font-medium opacity-80 sm:text-xl md:text-2xl lg:text-3xl">
          My connections
        </h1>
        ...
        {dataLoading ? (
          ...
          <div className="w-full overflow-x-auto pb-2 max-md:w-[clamp(20rem,-1.75rem+100vw,47.9375rem)]">
            <div className="min-w-[550px] overflow-hidden rounded-xl border border-(--table-border-clr) bg-(--white)">
              <table border="1" className="w-full">
                <thead>
                  <tr className="bg-(--accent-color)/40">
                    <th className="border-b border-(--table-border-clr) px-4 py-2 font-normal">
                      No.({connections.length})
                    </th>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="border-t border-(--table-border-clr) pl-4">
                          <span className="flex w-fit items-center gap-1 rounded-md bg-white py-0.5 pr-1.5 pl-1 text-xs shadow ring-1 ring-zinc-500/10">
                            <span
                              className={`size-[6px] rounded-full ${studyMode === 'Online' ? 'bg-emerald-300' : 'bg-zinc-500/40'}`}
                            ></span>
                            <span>{studyMode}</span>
                          </span>
                        </td>
                        ...
                        <td className="border-t border-(--table-border-clr)">
                          <div className="flex justify-center gap-0.5">
                            ...
                            <button className="group grid size-6 place-items-center rounded-md transition-colors duration-150 max-sm:size-7 pointer-fine:hover:bg-(--accent-color)">
                              <Pen
                                size="14"
                                className="opacity-60 group-hover:opacity-80"
                              />
      ...
      <AnimatePresence>
        {deleteConnection && (
          <motion.div
            ...
            className="fixed inset-0 z-25 flex items-center justify-center bg-zinc-500/50 p-4"
          >
            <motion.div
              initial={{
                scale: 1.2,
            ...
            >
              ...
              <div className="mt-4 flex items-center justify-end gap-2">
                <button
                  ...
                  className="h-9 w-24 rounded-full bg-(--white) text-sm tracking-wide shadow"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="grid h-9 w-24 place-items-center rounded-full bg-red-300 text-sm tracking-wide shadow"
                >
                  {deleting ? (
